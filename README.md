...
The TUI is arranged as `catalog | document`, with a related-file tree on the document's right edge. The catalog is one tree: a heading per author (or `local`, `unindexed`, `project only`) with its skills beneath. Use `h/j/k/l` or the arrow keys to focus and navigate panels, `gg`/`G` to jump to boundaries, and `x` to expand the focused pane. Headings are selectable: `space` toggles every skill in the group, `z` or `enter` folds it, `Z` folds or unfolds all, `h` jumps from a skill to its heading and folds an open heading, and `l` unfolds a folded one before moving right. Filtering with `/` shows every matching group open and hides the rest; the fold state returns when the filter clears. Press `v` or `V` to cycle between the split, catalog only, and document only, and `<`/`>` to resize the split. The focused pane is the one whose uppercase title is highlighted; zooming the document with `x` persists while you move between it and the file tree. `SKILL.md` frontmatter is hidden from the rendered document; press `f` to show it as a yaml block. On launch the TUI fetches the store's tracking branch in the background; when commits are waiting, the tab row shows `⇣ N to pull · S sync`, and `S` runs the full `slinky sync` in a modal with its output. Press `t` to pick a colour theme: moving through the list previews it live, `enter` records it in `~/.config/slinky/config.json`, and `esc` restores the saved one. Press `e` on a local catalog skill or an unindexed local skill to suspend the TUI and open that skill in your editor (the recorded `editor`, then `$VISUAL`, `$EDITOR`, then `nvim`) with the skills host as the working directory; vendor baselines, the temporary staging inbox, and project-only copies are intentionally excluded.
...
In the TUI, group headings show a yellow `⚠` when any of their visible skills has confirmed drift. Select a drifting vendor skill and press `d`. From the drift review, press `a` to accept the live global copy as the repository baseline, `r` to restore the global copy from the baseline, `h` to review in Hunk, or `d` to review in Delta.
