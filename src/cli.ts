...
import { adoptSkill, finalizeAdoption, findForeign, rollbackAdoption } from "./lib/adopt.ts";
...
import { alignStateWithManifest, getSkill, isSkillEnabled, loadManifest, loadState, saveManifest, saveState, withManifestSkill } from "./lib/manifest.ts";
import type { Manifest, State } from "./lib/manifest.ts";
import { AGENTS_SKILLS, HOME, isRepoDir, REPO, repoFound, repoResolutionError, saveHostConfig, SLINKY_CONFIG } from "./lib/paths.ts";
...
function cmdStatus(manifest: Manifest, state: State): void {
  ...
  for (const [name, meta] of Object.entries(manifest.skills)) {
    ...
    if (!enabled) {
      ...
      liveLabel = contentHash(join(AGENTS_SKILLS, name)) === meta.contentHash ? c.green("ok") : c.yellow("drift");
    } else {
...
function cmdCloneBootstrap(url: string, flagSet: Set<string>, extraArgs: string[]): never {
  ...
  const passthrough = ["--adopt-all", "--no-backup", "--dry-run", "--force"].filter((f) => flagSet.has(f));
...
const assertFlags = (allowed: ReadonlyArray<string>): void => {
  const unknown = [...flags].filter((flag) => !allowed.some((option) => (option.endsWith("=") ? flag.startsWith(option) : flag === option)));
...
async function main(initialManifest: Manifest, initialState: State): Promise<void> {
  ...
  switch (cmd) {
    ...
    case "disable": {
      ...
      renderAction(
        setSkillsEnabled(args, cmd === "enable", {
